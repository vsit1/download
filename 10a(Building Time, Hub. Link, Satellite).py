# Building Time, Hub. Link, Satellite
import sys
import os
...
base = datetime(2022,11,30,0,0,0)
numUnits=10*365*24
...
for i in date_list:
    ...
    if t==1:
        TimeFrame = pd.DataFrame.from_records(TimeLine) 
    else:
        TimeRow = pd.DataFrame.from_records(TimeLine)
...
for zone in active_timezones: 
    ...
    for j in range(TimeFrame.shape[0]): 
        ...
        if t==1:
            TimeZoneFrame = pd.DataFrame.from_records(TimeZoneLine) 
        else:
            TimeZoneRow = pd.DataFrame.from_records(TimeZoneLine)
